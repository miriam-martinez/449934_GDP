...
RAAN0_variation = [0 45 90 135 180 225 270 315];

visited = zeros(length(RAAN0_variation),n_debris);
revisited = zeros(length(RAAN0_variation),n_debris);
TimeObserved = cell(length(RAAN0_variation),n_debris);
for raan_idx = 1:length(RAAN0_variation)
    RAAN0_rad = deg2rad(RAAN0_variation(raan_idx));
            ...
            for j = size(possibledebris,2):-1:1
                    ...
                    if angle<=cone_half_rad && angle>=-cone_half_rad  && norm(d)<=maxrange_rad && norm(d)>=min_r_rad
                        ...
                        if t-FirstTimeVisitedDebris(idx2)>=0.125
                            visited(raan_idx,idx2) = visited(raan_idx,idx2)+1;
                            %coord_deb_cam = [];
                                ...
                                for nsc = 1:1:n_sats_cam
                                    ...
                                    if angle2<=cone_half_cam && angle2>=-cone_half_cam && norm(d2)<=maxrange_cam && norm(d2)>=minrange_cam
                                        ...
                                        if time-firsttime(idx2)==10
                                            revisited(raan_idx,idx2) = revisited(raan_idx,idx2) + 1;
                                            fprintf("he entrado \n");
                                            TimeObserved{raan_idx,idx2} = [time, TimeObserved{idx2}];
                                            break
                                        end
                                    end
                                end
