%% Full constellation - RADAR INCLINATION EFFECT
clear; close all; clc;
...
i_variation = [80 85 90 95 100];

visited = zeros(length(i_variation),n_debris);
revisited = zeros(length(i_variation),n_debris);
TimeObserved = cell(length(i_variation),n_debris);
for i_idx = 1:length(i_variation)
    inc_rad = deg2rad(i_variation(i_idx));

    RAAN_dot_rad = -3*pi*J2*Re^2*cos(inc_rad)./(H_sat_rad+Re).^2; % [rad/s]
    n_sats_rad = 3; % Fixed before

    RAAN_dot_rad_concat=[];
    theta0_rad_concat=[];
    for idx11=1:layers_rad
        RAAN_dot_rad_concat=[RAAN_dot_rad_concat,repmat(RAAN_dot_rad(idx11),1,n_sats_rad)];
        theta0_rad = linspace(0,2*pi,n_sats_rad);
            ...
            for j = size(possibledebris,2):-1:1
                    ...
                    if angle<=cone_half_rad && angle>=-cone_half_rad  && norm(d)<=maxrange_rad && norm(d)>=min_r_rad
                        ...
                        if t-FirstTimeVisitedDebris(idx2)>=0.125
                            visited(i_idx,idx2) = visited(i_idx,idx2)+1;
                                ...
                                for nsc = 1:1:n_sats_cam
                                    ...
                                    if angle2<=cone_half_cam && angle2>=-cone_half_cam && norm(d2)<=maxrange_cam && norm(d2)>=minrange_cam
                                        ...
                                        if time-firsttime(idx2)==10
                                            revisited(i_idx,idx2) = revisited(i_idx,idx2) + 1;
                                            fprintf("he entrado \n");
                                            TimeObserved{i_idx,idx2} = [time, TimeObserved{idx2}];
                                            break
                                        end
                                    end
                                end
